    ...
    for ep in range(episodes):
        ...
        if mastery_ready and (total_steps - last_mastery_eval_at >= mastery_eval_every):
            ...
            if writer:
                ...
                langevin_stats = agent.get_langevin_stats()
                writer.add_scalar('langevin/goals_sampled', langevin_stats['langevin_goals'], total_steps)
                writer.add_scalar('langevin/success_rate', langevin_stats['langevin_success_rate'], total_steps)
        ...
        if ep > 200 and ep % 100 == 0:
            recent_sr = csv_tracker.current_sr()
            if recent_sr < 0.05 and buffer_stats['positive_count'] < 50:
                print(f"\n[WARNING] Potential collapse detected at ep {ep}:")
                print(f"  Success rate: {recent_sr:.3f}")
                print(f"  Positive samples: {buffer_stats['positive_count']}")
                print(f"  Avg reward: {buffer_stats['avg_reward']:.3f}")
                print(f"  Consider checking hyperparameters or resetting.\n")
...
if __name__ == "__main__":
    ...
    tracker = CSVTracker("wvf_langevin_training")

  
    print("\n" + "="*60)
    print("Starting WVF Training with Langevin Goal Sampling")
    print("="*60)
    ...
    print(f"Langevin sampling: ENABLED")
    print("="*60 + "\n")
    ...
    print(f"  Total transitions: {final_stats['size']:,}")
    print(f"  Positive samples: {final_stats['positive_count']:,} ({final_stats['positive_ratio']:.2%})")
    print(f"  Average reward: {final_stats['avg_reward']:.3f}")
    print(f"  Reward range: [{final_stats['min_reward']:.2f}, {final_stats['max_reward']:.2f}]")
    print(f"\nInternal goals discovered: {len(agent.internal_goals)}")
    print(f"Done action ratio: {agent.get_done_action_ratio():.3f}")
    
    langevin_stats = agent.get_langevin_stats()
    print(f"\nLangevin Sampling Statistics:")
    print(f"  Goals sampled: {langevin_stats['langevin_goals']}")
    print(f"  Success rate: {langevin_stats['langevin_success_rate']:.3f}")
